...
function importerRowReadiness(row) {
  ...
  if (row.dataset.retailerId === "aldi" && row.dataset.storeSourceConfirmed !== "true") reasons.push("location_confirmation_required");
...
function renderCategoryUrlPreview(data) {
  ...
  productUrlAnalysis = data;
  preview.hidden = false;
  const locationValue = category.location?.confidence || "unknown";
  const productRows = (category.products || []).map((product, index) => {
    ...
    const locationConfirmed = ["confirmed_janesville", "confirmed_store_source"].includes(product.location?.confidence || category.location?.confidence);
    ...
    return `<article class="importer-product-row ${selected ? "is-selected" : ""} ${needsCriticalDetails ? "needs-details" : ""}" data-category-product="${index}" data-import-key="${escapeHtml(importerRequestKey(index))}" data-has-duplicates="${duplicates.length > 0}" data-retailer-recognized="${category.retailer?.recognized === true}" data-retailer-id="${escapeHtml(category.adapter || category.retailer?.retailer || "")}" data-approval-ready="${initialReadiness.ready === true}" data-store-source-confirmed="${storeConfirmed}">
      <div class="importer-product-main">
        ...
        <div class="importer-store"><strong>${escapeHtml(category.retailer?.retailer_name || "Retailer not recognized")}</strong>${priceSource.retailer_store_id ? `<span>Store #${escapeHtml(priceSource.retailer_store_id)}</span>` : ""}<span class="importer-location-badge location-${escapeHtml(storeSourceConfirmed ? "confirmed_store_source" : storeConfirmed ? "confirmed_janesville" : locationValue)}">${storeSourceConfirmed ? "Store source confirmed" : storeConfirmed ? "Store confirmed" : "Store not confirmed"}</span></div>
      ...
      <section class="importer-edit-panel" id="import-details-${index}" hidden>
        ...
        <div class="importer-edit-grid">
          ...
          <label><span>Product discovery source</span><input type="url" readonly value="${escapeHtml(category.source_url || "")}"></label>
          <label><span>Price source</span><input type="url" readonly value="${escapeHtml(priceSource.url || "Generic retailer page")}"><small>${priceSource.retailer_store_id ? `Walmart store #${escapeHtml(priceSource.retailer_store_id)} · retrieved ${escapeHtml(priceSource.retrieved_at || "")}` : "The retailer source did not establish an exact store."}</small></label>
...
async function analyzeProductUrl(event) {
  ...
  let retailerLabel = "retailer";
  try { retailerLabel = new URL(urlValue).hostname.replace(/^www\./, "").split(".")[0] || "retailer"; } catch {}
  button.disabled = true;
  button.classList.add("is-loading");
  if (buttonLabel) buttonLabel.textContent = `Analyzing ${titleCase(retailerLabel)} page…`;
  setMessage(message, `Analyzing ${titleCase(retailerLabel)} page…`, "info");
  try {
...
function categoryImportPayload(card) {
  ...
  const fields = source.fields || {};
  const priceSource = source.price_source || {};
  const values = collectImporterRowData(card);
  return {
    ...values,
    ...
    price_source_type: priceSource.type || "generic_retailer_web",
    price_source_url: priceSource.url || "",
    price_source_store_id: priceSource.retailer_store_id || "",
    retailer_store_slug: priceSource.retailer_store_slug || "",
    price_retrieved_at: priceSource.retrieved_at || ""
  };
}
