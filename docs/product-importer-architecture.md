...
ALDI collection prices require a matching `shopId`, postal code, and `zoneId`. The importer reads all three from the retailer's serialized storefront state. When the remote server defaults to another region, the Janesville-only importer requests the last verified Janesville context (`31930`, `53546`, `797`) and leaves exact-store confirmation to the administrator. The retailer API may change these identifiers; incomplete GraphQL price results stay in review. For variable-weight items, the estimated package total is retained as evidence while `price` uses the per-pound amount only when the estimate agrees with the supplied weight. Conflicts remain without an approvable price.
