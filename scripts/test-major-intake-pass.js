...
async function main() {
  ...
  try {
    ...
    const expiredOnly = await runDb(bulkDb, "INSERT INTO products (canonical_name,display_name,category,default_size_text,status,created_at,updated_at) VALUES ('expired only special','Expired Only Special','prepared food',NULL,'active',?,?)", [productNow, productNow]);
    ...
    await runDb(bulkDb, "INSERT INTO price_reports (user_id,store_id,product_id,item_name,category,price,sale_price,quantity,unit,unit_price,proof_type,confidence,status,price_type,valid_from_date,valid_through_date,promotion_conditions,promotion_schedule_text,display_offer_text,submitted_at,expires_at) VALUES (?,?,?,'Expired Only Special','prepared food',1.59,1,1,'each',1.59,'weekly_ad','high','approved','one_day_sale','2020-08-11','2020-08-11','Tuesday only','TODAY ONLY','$1.59',?,'2020-08-11T23:59:59.000Z')", [ownerRow.id, kwikTrip.id, expiredOnly.lastID, productNow]);
    ...
    assert.equal((await fetch(`${app.baseUrl}/`)).status, 200, "The public homepage must be anonymous.");
    const expiredBrowse = await fetch(`${app.baseUrl}/api/browse`).then((result) => result.json()); const expiredBrowseProduct = expiredBrowse.products.find((product) => Number(product.id) === expiredOnly.lastID); assert.ok(expiredBrowseProduct, "Expired-only products remain in public Explore."); assert.equal(expiredBrowseProduct.has_current_price, false); assert.equal(expiredBrowseProduct.best_price, null); assert.equal(expiredBrowseProduct.best_price_size_text, null); assert.equal(expiredBrowseProduct.best_report_id, null); assert.equal(expiredBrowseProduct.best_price_label, "Price needed");
    const expiredSearch = await fetch(`${app.baseUrl}/api/search?q=expired`).then((result) => result.json()); assert.ok(expiredSearch.products.some((product) => Number(product.id) === expiredOnly.lastID)); assert.equal(expiredSearch.reports.some((report) => Number(report.product_id) === expiredOnly.lastID), false);
    const expiredDetailResponse = await fetch(`${app.baseUrl}/api/products/${expiredOnly.lastID}`); const expiredDetail = await expiredDetailResponse.json(); assert.equal(expiredDetailResponse.status, 200); assert.equal(expiredDetail.product.best_price, null); assert.equal(expiredDetail.reports.length, 0);
    assert.equal((await fetch(`${app.baseUrl}/api/stores/${kwikTrip.id}`)).status, 200, "Store detail must be anonymous."); assert.equal((await fetch(`${app.baseUrl}/api/search?sort=newest_report`)).status, 200, "Deals/search data must be anonymous.");
