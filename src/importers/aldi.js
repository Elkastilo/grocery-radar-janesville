...
function collectionContext(html, pageUrl) {
  const fallback = { slug: new URL(pageUrl).pathname.split("/").filter(Boolean).pop() || "", shopId: "", postalCode: "53546", zoneId: "797" };
...
function normalizeAldiItem(item, pageUrl) {
  ...
  return {
    ...
    warnings: [price === null ? "Price was not present in the ALDI collection data." : "", image ? "" : "Image source was not present."].filter(Boolean)
  };
}
