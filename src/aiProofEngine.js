...
function normalizeAiResult(raw = {}, proof = {}) {
  const items = (Array.isArray(raw.items) ? raw.items : []).slice(0, 250).map((item, index) => {
    ...
    return {
      ...
      department: clean(item.department, 80),
      aisle: clean(item.aisle, 40),
      shelf: clean(item.shelf, 40),
      bay: clean(item.bay, 40),
      location_note: clean(item.location_note, 240),
    ...
    schema: {
      ...
      properties: {
        ...
        items: { type: "array", items: {
          ...
          required: ["raw_text", "normalized_name", "brand", "variant", "quantity", "package_size", "price", "unit_price", "price_basis", "comparison_price", "comparison_unit", "estimated_item_price", "approximate_item_weight", "approximate_item_weight_unit", "package_price", "multi_buy_quantity", "multi_buy_total", "category", "storage_type", "price_type", "valid_from_date", "valid_through_date", "promotion_conditions", "promotion_schedule_text", "display_offer_text", "department", "aisle", "shelf", "bay", "location_note", "date_evidence", "existing_product_match_id", "existing_product_match_confidence", "suggested_new_product", "confidence", "field_confidences", "warnings", "research_notes", "research_sources"],
          properties: {
            ...
            department: { type: "string" }, aisle: { type: "string" }, shelf: { type: "string" }, bay: { type: "string" }, location_note: { type: "string" },
